...
// ** Types
import { ContactType, ChatSidebarLeftType, ChatsArrType } from 'src/models/chatTypes'
    ...
    if (store && store.chats && store.chats.length) {
      if (query.length && !filteredChat.length) {
        ...
        return arrToMap.map((chat: ChatsArrType, index: number) => {
          const activeCondition = active !== null && active.id === chat.sentFromUserId && active.type === 'chat'

          return (
            <ListItem key={index} disablePadding sx={{ '&:not(:last-child)': { mb: 1 } }}>
              <ListItemButton
                disableRipple
                onClick={() => handleChatClick('chat', chat.sentFromUserId)}
                sx={{
                  py: 2,
                  px: 3,
                  width: '100%',
              ...
              >
                ...
                >
                  ...
                  >
                    <>{formatDateToMonthShort(chat.lastDateMessage as string, true)}</>
                  </Typography>
                  {chat.lastMassage && chat.unReadMessageCount > 0 ? (
                    <Chip
                      color='error'
                      label={chat.unReadMessageCount}
                      sx={{
                        mt: 0.5,
                        height: 18,
                        fontWeight: 600,
